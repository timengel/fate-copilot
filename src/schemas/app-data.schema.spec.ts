...
const fullCharacter: Character = {
  ...
  skills: [{ skill: 'Kämpfen', level: 3 }, { skill: 'Athletik', level: 2 }],
  stunts: [{ name: 'Schneller Schlag', description: '+2 auf Kämpfen im ersten Zug' }],
  extras: 'Magischer Stab',
  stressPhysical: [{ value: 1, checked: false }, { value: 2, checked: true }],
...
describe('app-data JSON Schema', () => {
  describe('valid AppData', () => {
    ...
    it('accepts all valid ConsequenceSeverity values (2, 4, 6, 8)', () => {
      ...
      expect(isValid({ ...minimalV11, characters: [{ ...minimalCharacter, consequences }] })).toBe(true)
    })
  ...
  describe('invalid Character', () => {
    ...
    it('rejects character with invalid type value', () => {
      expect(isValid({ ...minimalV11, characters: [{ ...minimalCharacter, type: 'player' }] })).toBe(false)
    })

    it('rejects character with invalid color value', () => {
      expect(isValid({ ...minimalV11, characters: [{ ...minimalCharacter, color: 'rot' }] })).toBe(false)
    })
    ...
    it('rejects Consequence with invalid severity', () => {
      const char = { ...minimalCharacter, consequences: [{ severity: 3, label: 'mild', value: '' }] }
      expect(isValid({ ...minimalV11, characters: [char] })).toBe(false)
    })

    it('rejects Consequence with invalid label', () => {
      const char = { ...minimalCharacter, consequences: [{ severity: 2, label: 'light', value: '' }] }
      expect(isValid({ ...minimalV11, characters: [char] })).toBe(false)
    })
  })
