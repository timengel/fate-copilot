...
describe('CheatSheetView', () => {
  ...
  it('hides the cheat sheet content when GM mode is disabled', () => {
    renderView(false);
    ...
    expect(screen.queryByRole('table', { name: 'Stufenleiter für Proben' })).toBeNull();
    expect(screen.queryByRole('heading', { level: 2, name: '4 Aktionen' })).toBeNull();
  });

  it('renders the updated cheat sheet sections and cards in GM mode', () => {
    renderView();
  ...
  it('renders ladder rows and the concise chances table in GM mode', () => {
    renderView();
  ...
  it('supports the action accordion toggling behavior in GM mode', async () => {
    renderView();
